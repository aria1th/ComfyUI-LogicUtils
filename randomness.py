...
class CounterFloat:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "reset": ("BOOLEAN"),
                "start": ("FLOAT", { "default": 0.0, "min": -float(0xffffffffffffffff), "max": float(0xffffffffffffffff), "step": 1.0, "display": "number" }),
                "step": ("FLOAT", { "default": 1.0, "min": -float(0xffffffffffffffff), "max": float(0xffffffffffffffff), "step": 1.0, "display": "number" }),
            },
        }
    RETURN_TYPES = ("FLOAT",)
    FUNCTION = "generate"
